...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTaghiAmount"))
        settings.setValue("nAnonymizeTaghiAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeTaghiAmount = settings.value("nAnonymizeTaghiAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeTaghiAmount"))
        SoftSetArg("-anonymizeTaghiamount", settings.value("nAnonymizeTaghiAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeTaghiAmount:
            nAnonymizeTaghiAmount = value.toInt();
            settings.setValue("nAnonymizeTaghiAmount", nAnonymizeTaghiAmount);
            emit anonymizeTaghiAmountChanged(nAnonymizeTaghiAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
