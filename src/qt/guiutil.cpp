...
{
    // Convert taghi:// to taghi:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("taghi://"))
    {
        uri.replace(0, 12, "taghi:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Taghi.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ITaghiLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ITaghiLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "taghi.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Taghi-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  taghi-qt [" + tr("command-line options") + "]                     " + "\n";
