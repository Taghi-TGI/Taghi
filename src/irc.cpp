...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #taghicoinTEST\r");
            Send(hSocket, "WHO #taghicoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #taghicoin\n");
            // randomly join #taghicoin00-#taghicoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #taghicoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #taghicoin%02d\r", channel_number).c_str());
        };
