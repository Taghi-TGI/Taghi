...
unsigned int nModifierInterval = 25 * 60; // time to elapse before new modifier is computed
...
const string strMessageMagic = "Taghi Signed Message:\n";
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  // 90% Annual interest 

    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = 25 * 60;  // 25 minutes for the timespan to adjust
