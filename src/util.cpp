...
int nAnonymizeTaghiAmount = 500000;
...
void RandAddSeedPerfmon()
{
    ...
    // This can take up to 2 seconds, so only do it every 5 minutes
    static int64_t nLastPerfmon;
    if (GetTime() < nLastPerfmon + 5 * 60)
        return;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Taghi
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Taghi
    // Mac: ~/Library/Application Support/Taghi
    // Unix: ~/.Taghi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Taghi";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "taghi.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "taghid.pid"));
